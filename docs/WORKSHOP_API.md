...
本文档同时描述仓库内可运行的零依赖演示服务，以及未来生产服务必须遵守的接口与安全边界。演示服务用于前端联调和交互展示，不承担真实账号、上传、审核或长期存储。
...
演示服务明确没有实现：账号、上传、数据库、对象存储、签名私钥、审核后台、全文检索、限流集群、邮件、付费、下载计数写入或内容自动扫描。生产环境不能把这个单文件服务直接暴露到公网。
...
| `WORKSHOP_ALLOWED_ORIGINS` | `*` | 逗号分隔的完整 Origin；生产环境必须设置明确白名单 |
| `WORKSHOP_MAX_BODY_BYTES` | `65536` | JSON 请求体上限，允许范围为 1 KiB 至 1 MiB |
...
8. 验证全部成功后原子写入 IndexedDB，并记录哪个地图引用该内容；
9. 在隔离的预览环境加载，发生错误时禁用该包而不是破坏存档；
10. 卸载时只删除没有被任何地图、存档或其他包引用的内容哈希。
